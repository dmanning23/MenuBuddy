...
namespace MenuBuddy
{
	...
	class ProfileSignInScreen : GameScreen
	{
		...
		public override void Update(GameTime gameTime, bool otherScreenHasFocus, bool coveredByOtherScreen)
		{
			...
			else if (IsActive && !Guide.IsVisible)
			{
				...
				else if (!haveShownGuide && !haveShownMarketplace)
				{
					...
					//Let's show it now, so they can sign in a profile.
		...
		{
			MessageBoxScreen confirmMarketplaceMessageBox = new MessageBoxScreen(
				"Online gameplay is not available in trial mode.\nWould you like to purchase this game?");
			confirmMarketplaceMessageBox.Accepted += PurchaseFullVersion;
